...
async def checkcpn(ctx):
    ...
    for coupon in data['data']:
        ...
        embed.add_field(
            name=f"Code: {coupon_id}\nCoupon Code: {coupon_code}",
            value=f"Discount: {discount}%\nProduct: {product_name}",
            inline=False
        )

    embed.set_footer(text="Powered by Deino0069", icon_url="https://cdn.discordapp.com/attachments/1325079943335378974/1338536131133702197/server_banner_2nd.png?ex=67ab706a&is=67aa1eea&hm=a90f60ae86d4f9b2cbc0a771767ebb06e46dd5df06e07f8e771cd34882dc3a9b&")
    
    await ctx.send(embed=embed)
@bot.command()
async def help(ctx):
    embed = discord.Embed(title="Help Menu", description="List of commands available", color=discord.Color.blue())
...
async def couponedit(ctx, id, CodeName, productid, discountamnt):
    ...
    if response.status_code == 200:
        ...
        embed.add_field(name="Edited Coupon: ", value=f"{id}", inline=False)
        embed.add_field(name="Discount: ", value=f"{discountamnt}%", inline=False)
        embed.set_footer(text="Powered by MR ALTS", icon_url="https://cdn.discordapp.com/avatars/1278919288870273024/a_0d6cf77b7d144815fb230d8103ae0296.gif?size=1024")
        await ctx.send(embed=embed)
@bot.command()
...
async def restocknotif(ctx):
    ...
    for product in data['data']:
        ...
        embed.add_field(
            ...
            value=f"**Stock:** {stock}\n"
                  f"**Price:** ${price}\n"
                  f"**Sold:** {products_sold}\n"
                  f"**Autobuy:** [Link]({autobuy_link})",  
            inline=False
        )

    embed.set_footer(text="Powered by Deino0069", icon_url="https://cdn.discordapp.com/avatars/1278919288870273024/a_0d6cf77b7d144815fb230d8103ae0296.gif?size=1024")
...
async def stock(ctx):
    ...
    embed.set_footer(text="Powered by MR Deino0069", icon_url="https://cdn.discordapp.com/avatars/1278919288870273024/a_0d6cf77b7d144815fb230d8103ae0296.gif?size=1024")
    await ctx.send(embed=embed)
@stock.error
    ...
    try:
        if '-' in numberofaccounts:
            ...
            if response_post.status_code == 500:
                embed = discord.Embed(title="Replace Product:", color=discord.Color.green())
                embed.add_field(name="User:", value=f"<@{ctx.author.id}>", inline=False)
                embed.add_field(name="Replaced: ", value=f"Number Of Account: {numberofaccounts}", inline=False)
    ...
    try:
        ...
        if len(message.attachments) > 0:
            ...
            if response.status_code == 200:
                ...
                embed.add_field(name="User:", value=f"<@{ctx.author.id}>", inline=False)
                embed.add_field(name="Restocked: ", value=f"Product: {productid}", inline=False)
                embed.add_field(name="Amount: ", value=f"{valid_lines_count}", inline=False)
                embed.set_footer(text="Powered by Deino0069", icon_url="https://cdn.discordapp.com/avatars/1278919288870273024/a_0d6cf77b7d144815fb230d8103ae0296.gif?size=1024")
                await ctx.send(embed=embed)
            else:
...
async def deletecoupon(ctx,id):
    ...
    if response.status_code == 200:
        embed = discord.Embed(title="```💴``` | Sellauth Coupon:", color=discord.Color.green())
        embed.add_field(name="User: ", value=f"<@{ctx.author.id}>", inline=False)
        embed.add_field(name="Deleted Coupon: ", value=f"{id}", inline=False)
        embed.set_footer(text="Powered by MR Deino0069", icon_url="https://cdn.discordapp.com/avatars/1278919288870273024/a_0d6cf77b7d144815fb230d8103ae0296.gif?size=1024")
        await ctx.send(embed=embed) 
@bot.command()
async def createcoupon(ctx, discount, code, *products):
    ...
    if response.status_code == 200:
        ...
        embed.add_field(name="=| User:", value=f"<@{ctx.author.id}>", inline=False)
        embed.add_field(name="| Coupon Created: ", value=f"{code}", inline=False)
...
async def update(ctx, product_id: int, new_price: str):
    ...
    if response.status_code == 200:
        ...
        if put_response.status_code == 200:
            ...
            embed.add_field(name="| Product Name:", value=f"{proudctname}", inline=False)
            embed.add_field(name="| New Price: ", value=f"{new_price}", inline=False)
...
async def bal(ctx):
    ...
    if ctx.author.id in bot.admins:
        ...
        embed.add_field(name="Wallet User:", value=f"MR ALTS", inline=False)
        embed.add_field(name="Balance (LTC)", value=f"{BalanceLTC:.6f} LTC", inline=False)
        embed.add_field(name="Balance (USD)", value=f"${BalanceUSD:.2f}", inline=False)
        embed.set_footer(text="Powered by MR ALTS", icon_url="https://cdn.discordapp.com/avatars/1278919288870273024/a_0d6cf77b7d144815fb230d8103ae0296.gif?size=1024")
        await ctx.send(embed=embed)
...
async def payout(ctx, ltcaddy: str, amount_in_usd: float):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            embed.add_field(name="LTC Address: ", value=f"{ltcaddy}", inline=False)
            embed.add_field(name="Requested Amount (LTC): ", value=f"{amount_in_ltc:.6f} LTC", inline=False)
            embed.add_field(name="Requested Amount (USD): ", value=f"${amount_in_usd:.2f}", inline=False)
            embed.add_field(name="Transaction ID: ", value=f"{transactionid}", inline=False)
            embed.set_footer(text="Powered by MR ALTS", icon_url="https://cdn.discordapp.com/avatars/1278919288870273024/a_0d6cf77b7d144815fb230d8103ae0296.gif?size=1024")
            await ctx.send(embed=embed)
            webhook_url = config["webhook1"]
            requests.post(webhook_url, data=json.dumps(log_data), headers={"Content-Type": "application/json"})
...
async def stats(ctx):
    ...
    embed.add_field(name=f"Total Orders: ", value=f"{total_orders}", inline=False)
    embed.add_field(name=f"Total Order Percentage: ", value=f"{total_percentage_orders}%", inline=False)
    embed.add_field(name=f"Total Revenue: ", value=f"${total_revenue}", inline=False)
    embed.add_field(name=f"Total Revenue Percentage: ", value=f"{total_percentage_revenue_change}%", inline=False)
    embed.add_field(name=f"Total Customers: ", value=f"{total_customers}", inline=False)
    embed.add_field(name=f"Total Customer Percentage: ", value=f"{total_customer_percntage_change}%", inline=False)
    await ctx.send(embed=embed)
@bot.command()
async def purchase(ctx, couponcode, email, amount):
    ...
    if response.status_code == 200 and 'url' in response.json():
        ...
        embed.add_field(name=f"Code: {couponcode}", value="", inline=False)
        embed.add_field(name=f"Coupon Code: {couponcode}", value="", inline=False)
        embed.add_field(name=f"Discount: N/A", value="", inline=False)
        embed.add_field(name=f"Product: Minecraft Java & Bedrock Edition [No Name Set]", value="", inline=False)
        embed.add_field(name=f"Email: {eemail}", value="", inline=False)
        embed.add_field(name=f"Invoice: ", value=f"[{invoice_id}](https://arenaboosts.mysellauth.com/invoice/show/{invoice_id})", inline=False)
        embed.add_field(name=f"Status: {status}", value="", inline=False)
        embed.add_field(name=f"Price: ${price}", value="", inline=False)
        embed.add_field(name=f"Currency: {currency}", value="", inline=False)
        embed.add_field(name=f"LTC Amount: {LTC_amount}", value="", inline=False)
